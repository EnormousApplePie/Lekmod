...
//	----------------------------------------------------------------------------
void CvCity::improvementAdjacency(ImprovementTypes eImprovement, ImprovementTypes eOtherImprovement, int iAmount, YieldTypes eYield, int iYieldChange, bool bBonusPerAmount)
{	

	VALIDATE_OBJECT
	CvAssertMsg(eImprovement > -1 && eImprovement < GC.getNumImprovementInfos(), "Invalid Improvement index.");
	CvAssertMsg(eOtherImprovement > -1 && eOtherImprovement < GC.getNumImprovementInfos(), "Invalid OtherImprovement index.");
	CvAssertMsg(eYield > -1 && eYield < NUM_YIELD_TYPES, "Invalid yield index.");

	// Loop through all the plots in the city and see if they have the improvement
	CvPlot* pLoopPlot;
	int iRange = CITY_PLOTS_RADIUS;
	...
	for (int iX = -iRange; iX <= iRange; iX++)
	{
		for (int iY = -iRange; iY <= iRange; iY++)
		{
			...
			if (pLoopPlot->getImprovementType() != eImprovement || pLoopPlot->getOwner() != getOwner())
			{
				continue;
			}

			int iCountAmount = 0;
			//loop trough each tile adjacent to this improvement
			for (int iI = 0; iI < NUM_DIRECTION_TYPES; iI++)
			{
				CvPlot* pAdjacentPlot = plotDirection(pLoopPlot->getX(), pLoopPlot->getY(), ((DirectionTypes)iI));
				if (pAdjacentPlot != NULL)
				{
					if (pAdjacentPlot->getImprovementType() == eOtherImprovement)
					{
						if (pAdjacentPlot->getOwner() == getOwner())
						{
							//count the amount of found eOtherImprovement
							iCountAmount++;
						}
					}
				}
			...
			// if the bonus is set to be per amount, then we multiply the yield change by the amount of eOtherImprovement
			if (bBonusPerAmount && (iCountAmount > 0 && iCountAmount <= iAmount))
			{
				iYieldChange = iYieldChange * iCountAmount;
				GET_PLAYER(getOwner()).changeImprovementYieldChange(pLoopPlot->getImprovementType(), eYield, iYieldChange);
			}
			// if the bonus is not set to be per amount, then it becomes a required amount for one yield change
			else if (!bBonusPerAmount && iCountAmount >= iAmount)
			{
				GET_PLAYER(getOwner()).changeImprovementYieldChange(pLoopPlot->getImprovementType(), eYield, iYieldChange);
			}
		}
