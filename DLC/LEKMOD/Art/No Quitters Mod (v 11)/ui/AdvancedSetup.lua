		...
		for i = 1, GameDefines.MAX_MAJOR_CIVS-1 do
			if( PreGame.GetSlotStatus( i ) ~= SlotStatus.SS_COMPUTER ) then
				...
				if( civIndex ~= -1 ) then
					...
					controlTable.CivPulldown:GetButton():LocalizeAndSetText("TXT_KEY_RANDOM_CIV_LEADER", Locale.ConvertTextKey(leaderDescription), Locale.ConvertTextKey(civ.ShortDescription));
...
function RefreshHumanPlayer()
    ...
    if( civIndex ~= -1 ) then
		...
		if(name ~= "") then
			...
			Controls.CivName:SetText( Locale.ConvertTextKey( "TXT_KEY_RANDOM_CIV_LEADER", name, civName ));
			...
			Controls.CivPulldown:GetButton():SetText( Locale.ConvertTextKey( "TXT_KEY_RANDOM_CIV_LEADER", Locale.ConvertTextKey( leaderDescription ), Locale.ConvertTextKey( civ.ShortDescription ) ) );
		end
	...
	FullSync = function()
		...
		function GetPlayableCivInfo()
			...
			table.sort(civs, function(a,b) return Locale.Compare(a.ShortDescription, b.ShortDescription) == -1; end); 
															  --(a.LeaderDescription, b.LeaderDescription) == -1; end);
		...
		function PopulateCivPulldown( playableCivs, pullDown, playerID )
			...
			for id, civ in ipairs(playableCivs) do
				...
				controlTable.Button:LocalizeAndSetText("TXT_KEY_RANDOM_CIV_LEADER", civ.LeaderDescription, civ.ShortDescription);
...
function OnCancelEditPlayerDetails()
    ...
    if( civIndex ~= -1 ) then
		...
		Controls.CivPulldown:GetButton():LocalizeAndSetText("TXT_KEY_RANDOM_CIV_LEADER", Locale.ConvertTextKey(leaderDescription), Locale.ConvertTextKey(civ.ShortDescription));
	else
		Controls.CivPulldown:GetButton():LocalizeAndSetText("TXT_KEY_RANDOM_CIV");
	end
end
