...
function GetHelpTextForUnit(iUnitID, bIncludeRequirementsInfo, pCity)
	...
	if(pUnitInfo.Cost > 0) then
		if pCity then
			strHelpText = strHelpText .. Locale.ConvertTextKey("TXT_KEY_PRODUCTION_COST", pCity:GetUnitProductionNeeded(iUnitID));
		else
...
function GetHelpTextForBuilding(iBuildingID, bExcludeName, bExcludeHeader, bNoMaintenance, pCity)
	...
	local iGlobalHappiness = pBuildingInfo.UnmoddedHappiness + pActivePlayer:GetTraitBuildingClassGlobalHappiness(buildingClassID)
		+ pActivePlayer:GetLegacyBuildingClassGlobalHappiness(buildingClassID);
	if (iGlobalHappiness ~= nil) then
		iHappinessTotal = iHappinessTotal + iGlobalHappiness;
	end
	iHappinessTotal = iHappinessTotal + pActivePlayer:GetExtraBuildingHappinessFromPolicies(iBuildingID) + pActivePlayer:GetTraitBuildingClassHappiness(buildingClassID)
		+ pActivePlayer:GetLegacyBuildingClassHappiness(buildingClassID);
	if (pCity ~= nil) then
	...
	-- START Yields, this should support all yields, including modded Yields as long as they are defined in the XML
	for Yields in GameInfo.Yields() do
    		local iYieldType          = Yields.ID;
    		local iYieldChange        = 0;
    		local iYieldModifier      = 0;
    		local fYieldChangePerPop  = 0;
			local strYieldName 		  = Locale.ConvertTextKey( Yields.Description );
    		local strYieldIcon        = Yields.IconString;
			local iTechEnhancedYield  = 0;
			local iEraEnhancedYield   = 0;
			local iTeamEra = pActiveTeam:GetCurrentEra();

    	iYieldChange = iYieldChange + Game.GetBuildingYieldChange(iBuildingID, iYieldType) + pActivePlayer:GetPolicyBuildingClassYieldChange(buildingClassID, iYieldType) + pActivePlayer:GetLegacyBuildingClassYieldChange(buildingClassID, iYieldType)
			+ pActivePlayer:GetTraitBuildingClassYieldChange(buildingClassID, iYieldType) + pActivePlayer:GetPlayerBuildingClassYieldChange(buildingClassID, iYieldType);
    	if pCity ~= nil then
    	    iYieldChange = iYieldChange + pCity:GetReligionBuildingClassYieldChange(buildingClassID, iYieldType) + pCity:GetLeagueBuildingClassYieldChange(buildingClassID, iYieldType);
    	end

    	iYieldModifier = iYieldModifier + Game.GetBuildingYieldModifier(iBuildingID, iYieldType) + pActivePlayer:GetPolicyBuildingClassYieldModifier(buildingClassID, iYieldType)
			+ pActivePlayer:GetTraitBuildingClassYieldModifier(buildingClassID, iYieldType) + pActivePlayer:GetLegacyBuildingClassYieldModifier(buildingClassID, iYieldType);
		-- Hotel, National Visitor Center and Airports
		local iLandmarkPercent = pBuildingInfo.LandmarksTourismPercent;
		local iTotalLandmark = 0;
		if (iLandmarkPercent ~= 0) and Yields.Type == "YIELD_TOURISM" then
			if (pCity ~= nil) then 
				local iCultureWonders = pCity:GetCultureFromWonders();
				local iCultureImprovements = pCity:GetCultureFromImprovements();
				local iCultureNW = pCity:GetCultureFromNaturalWonders();
		 		iTotalLandmark = ((iCultureWonders + iCultureImprovements + iCultureNW) * iLandmarkPercent / 100 )
			end
		end
    	for row in GameInfo.Building_YieldChangesPerPop{ BuildingType = pBuildingInfo.Type, YieldType = Yields.Type } do
    	    fYieldChangePerPop = fYieldChangePerPop + (row.Yield or 0);
    	end
		for row in GameInfo.Building_TechEnhancedYieldChanges{ BuildingType = pBuildingInfo.Type, YieldType = Yields.Type } do
			iTechEnhancedYield = iTechEnhancedYield + (row.Yield or 0);
		end
		for row in GameInfo.Building_EraEnhancedYieldChanges{ BuildingType = pBuildingInfo.Type, YieldType = Yields.Type } do
		    local eRowEra = GameInfoTypes[row.EraType]
		    if eRowEra and iTeamEra >= eRowEra then
		        iEraEnhancedYield = iEraEnhancedYield + (row.Yield or 0);
		    end
		end
		-- I'll get Firaxis for this later....
		if Yields.Type == "YIELD_CULTURE" then
			-- Culture is special, for better or worse
			local iBuildingCultureModifier = pBuildingInfo.CultureRateModifier + pBuildingInfo.GlobalCultureRateModifier;
			if iBuildingCultureModifier ~= 0 then
				iYieldModifier = iYieldModifier + iBuildingCultureModifier;
			end
		end
		-- This Creates a new line for each yield application type, since stacking them up looks bad
		if pActiveTeam:GetTeamTechs():HasTech(iEnhancingTech) then 
			iYieldChange = iYieldChange + iTechEnhancedYield; -- Combine Tech Yield and Base Yield, if you have the tech.
		end
		if iEraEnhancedYield ~= 0 then
			iYieldChange = iYieldChange + iEraEnhancedYield;
		end
		if iTotalLandmark ~= 0 then
			iYieldChange = iYieldChange + iTotalLandmark;
		end
    	if iYieldChange ~= 0 then
    	    table.insert(lines, strYieldIcon .. " " .. strYieldName .. string.format(": %+i", iYieldChange))
    	end
    	if iYieldModifier ~= 0 then
    	    table.insert(lines, strYieldIcon .. " " .. strYieldName .. string.format(": %+i%%", iYieldModifier))
    	end
    	if fYieldChangePerPop ~= 0 then
			if (pCity) then
				local iPopulation = pCity:GetPopulation();
				fYieldChangePerPop = (fYieldChangePerPop * iPopulation) / 100;
			else
				fYieldChangePerPop = fYieldChangePerPop / 100;
			end
    	    table.insert(lines, strYieldIcon .. " " .. strYieldName .. string.format(": %+g[ICON_CITIZEN]", fYieldChangePerPop))
    	end
	end -- END Yields

	-- Great People
	local greatPersonRate = pBuildingInfo.GreatPeopleRateModifier;
	if (greatPersonRate ~= 0) then
		table.insert(lines, "[ICON_GREAT_PEOPLE] " .. string.format(": %+i%%", greatPersonRate)); 
	end
...
function GetCultureTooltip(pCity)
	...
	if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_POLICIES)) then
		...
		local iYieldType = YieldTypes.YIELD_CULTURE;
		strCultureToolTip = strCultureToolTip .. GetYieldTooltipHelper(pCity, iYieldType, "[ICON_CULTURE]");
	end
...
function GetFaithTooltip(pCity)
	-- Faith is a bit special, since it also shows Religious Followers breakdown
	local iYieldType = YieldTypes.YIELD_FAITH
	local faithTips = {}
	
	if Game.IsOption(GameOptionTypes.GAMEOPTION_NO_RELIGION) then
		return Locale.ConvertTextKey("TXT_KEY_TOP_PANEL_RELIGION_OFF_TOOLTIP")
	end

	-- Basic help
	if not OptionsManager.IsNoBasicHelp() then
		table.insert(faithTips, Locale.ConvertTextKey("TXT_KEY_FAITH_HELP_INFO"))
		table.insert(faithTips, "[NEWLINE]")
	end
	
	table.insert(faithTips, GetYieldTooltipHelper(pCity, iYieldType, "[ICON_PEACE]"))
...
-- v34 Golden Age Points
function GetGoldenAgePointsTooltip(pCity)

	local strGoldenAgePointsToolTip = "";

	local iYieldType = YieldTypes.YIELD_GOLDEN_AGE_POINTS;

	if (not OptionsManager.IsNoBasicHelp()) then
		strGoldenAgePointsToolTip = strGoldenAgePointsToolTip .. Locale.ConvertTextKey("TXT_KEY_GOLDEN_AGE_POINTS_HELP_INFO");
		strGoldenAgePointsToolTip = strGoldenAgePointsToolTip .. "[NEWLINE][NEWLINE]";
	end

	strGoldenAgePointsToolTip = strGoldenAgePointsToolTip .. GetYieldTooltipHelper(pCity, iYieldType, "[ICON_GOLDEN_AGE]");
	
	return strGoldenAgePointsToolTip;
end

-- TOURISM
function GetTourismTooltip(pCity)
	local strTourismToolTip = "";

	local iYieldType = YieldTypes.YIELD_TOURISM;

	if (not OptionsManager.IsNoBasicHelp()) then
		strTourismToolTip = strTourismToolTip .. Locale.ConvertTextKey("TXT_KEY_TOURISM_HELP_INFO");
		strTourismToolTip = strTourismToolTip .. "[NEWLINE][NEWLINE]";
	end

	strTourismToolTip = strTourismToolTip .. GetYieldTooltipHelper(pCity, iYieldType, "[ICON_TOURISM]");
...
function GetYieldTooltip(pCity, iYieldType, iBase, iTotal, strIconString, strModifiersString)
	...
	if (iYieldFromGreatWorks ~= 0) then
		strYieldBreakdown = strYieldBreakdown .. "[ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_YIELD_FROM_GREAT_WORKS", iYieldFromGreatWorks, strIconString);
		strYieldBreakdown = strYieldBreakdown .. "[NEWLINE]";
	end
	-- Start 34.8
	local iYieldFromTrait = pCity:GetBaseYieldRateFromTraits(iYieldType);
	if (iYieldFromTrait ~= 0) then
		strYieldBreakdown = strYieldBreakdown .. "[ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_YIELD_FROM_TRAITS", iYieldFromTrait, strIconString);
		strYieldBreakdown = strYieldBreakdown .. "[NEWLINE]";
	end
	local iYieldFromLeagues = pCity:GetBaseYieldRateFromLeagues(iYieldType);
	if (iYieldFromLeagues ~= 0) then
		strYieldBreakdown = strYieldBreakdown .. "[ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_YIELD_FROM_LEAGUES", iYieldFromLeagues, strIconString);
		strYieldBreakdown = strYieldBreakdown .. "[NEWLINE]";
	end
	local iYieldFromPolicies = pCity:GetBaseYieldRateFromPolicies(iYieldType);
	if (iYieldFromPolicies ~= 0) then
		strYieldBreakdown = strYieldBreakdown .. "[ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_YIELD_FROM_POLICIES", iYieldFromPolicies, strIconString);
		strYieldBreakdown = strYieldBreakdown .. "[NEWLINE]";
	end
	local iYieldFromGarrison = pCity:GetBaseYieldRateFromGarrison(iYieldType);
	if (iYieldFromGarrison ~= 0) then
		strYieldBreakdown = strYieldBreakdown .. "[ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_YIELD_FROM_GARRISON", iYieldFromGarrison, strIconString);
		strYieldBreakdown = strYieldBreakdown .. "[NEWLINE]";
	end
	local iYieldFromThemedBuildings = pCity:GetBaseYieldRateFromThemedBuildings(iYieldType);
	if (iYieldFromThemedBuildings ~= 0) then
		strYieldBreakdown = strYieldBreakdown .. "[ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_YIELD_FROM_THEMED_BUILDINGS", iYieldFromThemedBuildings, strIconString);
		strYieldBreakdown = strYieldBreakdown .. "[NEWLINE]";
	end
	local iYieldFromLandmarks = pCity:GetBaseYieldRateFromLandmarks(iYieldType);
	if (iYieldFromLandmarks ~= 0) then
		strYieldBreakdown = strYieldBreakdown .. "[ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_YIELD_FROM_LANDMARKS", iYieldFromLandmarks, strIconString);
		strYieldBreakdown = strYieldBreakdown .. "[NEWLINE]";
	end
	-- End 34.8
	if (iYieldType == YieldTypes.YIELD_FOOD) then
	...
	-- Food Production, if applicable. Not Modified so put at the end.
	if (iYieldType == YieldTypes.YIELD_PRODUCTION) then
		if pCity:IsFoodProduction() then
			strYieldBreakdown = strYieldBreakdown .. Locale.ConvertTextKey("TXT_KEY_PRODMOD_FOOD_CONVERSION", pCity:GetFoodProduction(pCity:GetYieldRate(YieldTypes.YIELD_FOOD, false) - pCity:FoodConsumption(true, 0)));
		end
	end
