...
local goldenageTexture = "yieldatlas_128_GA.dds"
...
function BuildYield( x, y, index )
    ...
    SetYieldIcon( 6, yieldInfo.m_GoldenAge,  anchor.Stack, record ); -- v34
...
function SetYieldIcon( yieldType, amount, parent, record )
    ...
    if amount > 0 then
       ...
       elseif (yieldType == 5) then
            ...
            -- v34 ADD GOLDEN AGE POINTS AS YIELD
        elseif (yieldType == 6) then
            imageInstance.Image:SetTexture(goldenageTexture);
            if( amount >= 5 ) then
                imageInstance.Image:SetTextureOffsetVal( 0 * 128, 512 );
            else
                imageInstance.Image:SetTextureOffsetVal( 0 * 128, 128 * ( amount - 1 ) );
...
function RecordYieldInfo( x, y, index, plot )
    ...
    local iGoldenAge  = plot:CalculateYield( 6, true ); -- v34
	    

    -- early out of allocations if there is no yield for this tile
    if( iFood == 0 and iProduction == 0 and iGold == 0 and iScience == 0 and iCulture == 0 and iFaith == 0 and iGoldenAge == 0) then -- v34
        return;
    end
