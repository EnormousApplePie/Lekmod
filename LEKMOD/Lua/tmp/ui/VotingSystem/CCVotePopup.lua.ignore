...
function DoDiploVoteChosen( iVotePlayer )
	print('--CC PLAYER CHOICE:', iVotePlayer)
	Network.SendGiftUnit(iVotePlayer, -3);
...
function OnPopupMessage(popupInfo)
	...
	for iTeamLoop = 0, GameDefines.MAX_CIV_TEAMS-1, 1 do
		...
		if(pOtherTeam ~= nil) then
			...
			if (pOtherPlayer ~= nil and pOtherPlayer:IsAlive() and pOtherPlayer:IsHuman()) then
				-- Not minor
...
LuaEvents.MPProposeCCButtonPress.Add( OnPopupMessage );
...
function ShowHideHandler( bIsHide, bInitState )
    ...
    if( not bInitState ) then
        if( not bIsHide ) then
            ...
            Events.SerialEventGameMessagePopupProcessed.CallImmediate(123, 0);
        end
    end
end
