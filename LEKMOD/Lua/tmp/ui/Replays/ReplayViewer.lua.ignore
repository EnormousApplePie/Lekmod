		...
		DrawGraph = function(panel)
			...
			for i,v in ipairs(playerInfos) do
				for turn, score in pairs(v.Scores) do
					...
					if(s and turn <= g_ReplayInfo.FinalTurn) then
						if(s > maxScore) then
							maxScore = s;
						end
			...
			function RefreshCivilizations()
				...
				function ColorDistance(color1, color2)
					...
					if(distance == nil) then
						...
						local a2 = pow((color1.Alpha - color2.Alpha)*255, 2);
						
						distance = r2 + g2 + b2 + a2;
				...
				--[[function DetermineGraphColor(playerColor)
					...
					elseif(IsUniqueColor(GameInfo.Colors[playerColor.SecondaryColor])) then
						...
						return {Type = "COLOR_GREEN", Red = 0, Green = 1, Blue = 0, Alpha = 1,};
					end--]]
				function DetermineGraphColor(i)
					local playernum = i;
					local hue = 0;
					if i % 2 == 0 then
						hue = ((30 * i/2 ) % 360)/360;
					else
						hue = ((180 + 45 * (i-1)/2) % 360) / 360;
					end
					return ColorFromHSL(hue, 1, .6);
				end
				...
				for i, player in ipairs(g_ReplayInfo.PlayerInfo) do
					...
					local color = DetermineGraphColor(i);
