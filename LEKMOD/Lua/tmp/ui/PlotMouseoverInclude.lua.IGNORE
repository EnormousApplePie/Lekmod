...
-------------------------------------------------
-------------------------------------------------
function GetYieldString(plot) -- v34. Changed this whole function lol.
	local bDebugOutput = false
	local strYield = ""

	local iPlotOwner = plot:GetOwner()
	local iActivePlayer = Game.GetActivePlayer()
	local pLocalPlayer = Players[iActivePlayer]
	local iLocalTeam = pLocalPlayer:GetTeam()

	-- Check for feature and Natural Wonder info
	local featureType = plot:GetFeatureType()
	local featureInfo = (featureType ~= FeatureTypes.NO_FEATURE) and GameInfo.Features[featureType] or nil
	local bIsNaturalWonder = featureInfo and featureInfo.NaturalWonder

	-- YIELDS
	for row in GameInfo.Yields() do 
		local iYieldID = row.ID
		local iAmount = 0

		if iPlotOwner ~= -1 then
			-- Owned: use normal game logic
			iAmount = plot:CalculateYield(iYieldID, true)
		elseif bIsNaturalWonder then
			-- Unowned Natural Wonder: simulate local player’s view
			iAmount = plot:CalculateNatureYield(iYieldID, iLocalTeam, false)
		else
			-- Unowned non-natural tile: use standard calculation
			iAmount = plot:CalculateYield(iYieldID, true)
		end

		if iAmount > 0 then 
			strYield = strYield .. row.IconString .. " " .. iAmount .. " "

			if bDebugOutput then
				print(string.format("Yield: %s (%d) = %d", row.Type, iYieldID, iAmount))
			end
		end
	end

	-- HAPPINESS
	if featureInfo and featureInfo.InBorderHappiness and featureInfo.InBorderHappiness > 0 then
		local plotHappiness = featureInfo.InBorderHappiness
		local iWonderMod = 0

		if bIsNaturalWonder then
			if iPlotOwner ~= -1 then
				iWonderMod = Players[iPlotOwner]:GetNaturalWonderYieldModifier()
			else
				iWonderMod = pLocalPlayer:GetNaturalWonderYieldModifier()
			end

			if iWonderMod > 0 then
				plotHappiness = math.floor(plotHappiness * (100 + iWonderMod) / 100)
			end
		end
...
--[[ Old implementation, keep just in case
	...
	-- Golden Age Points v34
	local iNumGoldenAge = plot:CalculateYield(6, true);
