...
function TipHandler( control )
    ...
    if (action.Type == "MISSION_ALERT" and not unit:IsEverFortifyable()) then
		...
		strToolTip = strToolTip .. strActionHelp;
	elseif (action.Type == "MISSION_BUY_CITY_STATE") then
		if (bFirstEntry) then
       		bFirstEntry = false;
    	else
        	strActionHelp = strActionHelp .. "[NEWLINE]";
		...
		strActionHelp = strActionHelp .. Locale.ConvertTextKey("TXT_KEY_MISSION_BUY_CITY_STATE_HELP");
		strToolTip = strToolTip .. strActionHelp;

		if bDisabled then
        	local thisUnitOwner = unit:GetOwner()
        	...
        	if not pMinor then
        	    ...
        	    strDisabledString = strDisabledString .. Locale.ConvertTextKey("TXT_KEY_BUY_CITY_STATE_HELP_DISABLED_TERRITORY")
        	else
        	    local ally = pMinor:GetAlly() -- -1 if no ally
				local csNameKey = pMinor:GetCivilizationShortDescriptionKey() 
        	    if ally ~= -1 and ally ~= thisUnitOwner then
        	        if (bFirstEntry) then
						bFirstEntry = false;
					elseif (not bFirstEntry) then
						strDisabledString = strDisabledString .. "[NEWLINE][NEWLINE]";
					end
        	        strDisabledString = strDisabledString .. Locale.ConvertTextKey( "TXT_KEY_BUY_CITY_STATE_HELP_DISABLED_ALLIED_TO_ANOTHER", csNameKey, Players[ally]:GetCivilizationShortDescriptionKey() )
        	    end

        	    local window = GameDefines.MINOR_CIV_BUYOUT_TURNS or 0
        	    if window > 0 and pMinor.GetLastAllyTurnWithMajor then
        	        local thisTurn = Game.GetGameTurn()
        	        for iMajor = 0, GameDefines.MAX_MAJOR_CIVS - 1 do
        	            if iMajor ~= thisUnitOwner then
        	                local lastTurn = pMinor:GetLastAllyTurnWithMajor(iMajor) or -1 -- -1 if never
        	                if lastTurn >= 0 then
        	                    local turnDelta = thisTurn - lastTurn
        	                    if turnDelta < window then
        	                        if (bFirstEntry) then
										bFirstEntry = false;
									elseif (not bFirstEntry) then
										strDisabledString = strDisabledString .. "[NEWLINE][NEWLINE]";
									end
        	                        strDisabledString = strDisabledString .. Locale.ConvertTextKey( "TXT_KEY_BUY_CITY_STATE_HELP_DISABLED_RECENT_ALLY", csNameKey, Players[iMajor]:GetCivilizationShortDescriptionKey(), window - turnDelta )
        	                        break
        	                    end
        	                end
        	            end
	...
	elseif (action.Type == "MISSION_RETRAIN") then
		if (bFirstEntry) then
       		bFirstEntry = false;
    	else
        	strActionHelp = strActionHelp .. "[NEWLINE]";
    	...
    	if iUnitType ~= -1 then
    	    local iGoldToUpgrade = unit:UpgradePrice(iUnitType)
    	    iRetrainCost = math.max(50, math.ceil(iGoldToUpgrade / 2))
    	else
    	    iRetrainCost = 50
    	end

		local iNumRefundedPromotions = unit:GetNumPlayerChosenPromotions() or 0
		local iRemovedXP = 15 * math.max(0, iNumRefundedPromotions)
    	local iRefundedXP = 15 * math.max(0, iNumRefundedPromotions - 1)
    	local iNewUnitLevel = unit:GetLevel() - iNumRefundedPromotions
    	if iNewUnitLevel < 1 then iNewUnitLevel = 1 end

		strActionHelp = strActionHelp .. Locale.ConvertTextKey("TXT_KEY_MISSION_RETRAIN_HELP", iRemovedXP, iRefundedXP, iNewUnitLevel, iRetrainCost);

		strToolTip = strToolTip .. strActionHelp;
		if bDisabled then
			local pActivePlayer = Players[Game.GetActivePlayer()];
			-- Can't Retrain because we lack the Gold
			if (iRetrainCost > pActivePlayer:GetGold()) then
				...
				strDisabledString = strDisabledString .. Locale.ConvertTextKey("TXT_KEY_RETRAIN_HELP_DISABLED_GOLD");
			end
			-- Can't upgrade because we're outside our territory
			if (pPlot:GetOwner() ~= unit:GetOwner()) then
				...
				strDisabledString = strDisabledString .. Locale.ConvertTextKey("TXT_KEY_RETRAIN_HELP_DISABLED_TERRITORY");
			end
			-- Can't Retrain because we do not have enough chosen Promotions
			if (iNumRefundedPromotions < 2) then
				-- Add spacing for all entries after the first
				if (bFirstEntry) then
					bFirstEntry = false;
				elseif (not bFirstEntry) then
				...
				strDisabledString = strDisabledString .. Locale.ConvertTextKey("TXT_KEY_RETRAIN_HELP_DISABLED_PROMOTIONS");
			end
			-- Can't Retrain because we Lack experience
			if (unit:GetExperience() < 30) then
				-- Add spacing for all entries after the first
				if (bFirstEntry) then
					bFirstEntry = false;
				elseif (not bFirstEntry) then
				...
				strDisabledString = strDisabledString .. Locale.ConvertTextKey("TXT_KEY_RETRAIN_HELP_DISABLED_EXPERIENCE");
			end
			strDisabledString = "[COLOR_WARNING_TEXT]" .. strDisabledString .. "[ENDCOLOR]"
		end
