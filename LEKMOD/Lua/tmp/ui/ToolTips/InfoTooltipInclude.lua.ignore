...
function GetCultureTooltip(pCity)
	...
	if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_POLICIES)) then
		...
		-- Culture from Per-Population Yields
		local iCulturePerPop = pCity:GetYieldPerPopTimes100(YieldTypes.YIELD_CULTURE) * pCity:GetPopulation() / 100;
		if (iCulturePerPop ~= 0) then
			...
			strCultureToolTip = strCultureToolTip .. "[ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_YIELD_FROM_POP_EXTRA", iCulturePerPop, "[ICON_CULTURE]");
		end
...
function GetGoldenAgePointsTooltip(pCity)
	...
	strGoldenAgePointsToolTip = strGoldenAgePointsToolTip .. GetYieldTooltipHelper(pCity, iYieldType, "[ICON_GOLDEN_AGE_POINTS]");
