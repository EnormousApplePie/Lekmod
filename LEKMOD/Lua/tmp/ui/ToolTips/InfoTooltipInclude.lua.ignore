...
function GetHelpTextForBuilding(iBuildingID, bExcludeName, bExcludeHeader, bNoMaintenance, pCity)
	...
	iHappinessTotal = iHappinessTotal + pActivePlayer:GetExtraBuildingHappinessFromPolicies(iBuildingID) + pActivePlayer:GetTraitBuildingClassHappiness(iBuildingID);
	if (pCity ~= nil) then
	...
	for Yields in GameInfo.Yields() do
			...
			local iEnhancedYieldTech  = GameInfo.Buildings[iBuildingID].EnhancedYieldTech;
			local iTechEnhancedYield  = 0;
			local iEraEnhancedYield   = 0;
			local iTeamEra = pActiveTeam:GetCurrentEra();

    	iYieldChange = iYieldChange + Game.GetBuildingYieldChange(iBuildingID, iYieldType) + pActivePlayer:GetPolicyBuildingClassYieldChange(buildingClassID, iYieldType) + pActivePlayer:GetTraitBuildingClassYieldChange(buildingClassID, iYieldType) + pActivePlayer:GetPlayerBuildingClassYieldChange(buildingClassID, iYieldType)
    	if pCity ~= nil then
		...
		if pActiveTeam:GetTeamTechs():HasTech(iEnhancedYieldTech) then 
	...
	if(iTechEnhancedTourism > 0 and pActiveTeam:GetTeamTechs():HasTech(iEnhancedYieldTech)) then
