...
function UpdateDisplay()
    ...
    for pCity in pPlayer:Cities() do
        ...
        sortEntry.Culture = pCity:GetJONSCulturePerTurnTimes100() / 100;
		...
		if(Game.IsOption(GameOptionTypes.GAMEOPTION_NO_HAPPINESS)) then
			instance.GoldenAgeBox:SetHide(true);
		else
			sortEntry.GoldenAge = pCity:GetYieldRate( YieldTypes.YIELD_GOLDEN_AGE_POINTS );
			instance.GoldenAge:SetText( sortEntry.GoldenAge );
			instance.GoldenAgeBox:SetHide(false);
		end
...
function SortFunction( a, b )
    ...
    if (entryA == nil) or (entryB == nil) then 
		...
		elseif( m_SortMode == eFaith ) then
			valueA = entryA.Faith;
			valueB = entryB.Faith;
		elseif( m_SortMode == eGoldenAge ) then
			valueA = entryA.GoldenAge;
			valueB = entryB.GoldenAge;
		else -- SortProduction
...
Controls.SortGoldenAge:RegisterCallback(Mouse.eLClick, OnSort);
Controls.SortGoldenAge:SetVoid1(eGoldenAge);
