...
function GatherInfoAboutUniqueStuff( civType )
	...
	for thisUnitClass in GameInfo.UnitClasses() do
		...
		-- MOD.EAP: Remove any units that are not allowed to be built for this player
		if validUnitBuilds[thisUnitClass.Type] == "UNIT_WORKBOAT" then
			validUnitBuilds[thisUnitClass.Type] = nil;
		end
		-- MOD.EAP: End
	end
	...
	for thisImprovement in GameInfo.Improvements() do
		...
		-- MOD.EAP: Remove any improvements that are not allowed for this civ
		for leader in GameInfo.Civilization_Leaders() do
			if leader.CivilizationType == civType then
				local leaderType = leader.LeaderheadType;
				for trait in GameInfo.Leader_Traits() do
					if trait.LeaderType == leaderType then
						local leaderTrait = trait.TraitType;
						if GameInfo.Trait_NoBuildImprovement == nil then
							break;
						end
						for noBuildImprovement in GameInfo.Trait_NoBuildImprovement() do
							if noBuildImprovement.TraitType == leaderTrait and noBuildImprovement.ImprovementType == thisImprovement.Type then
		...
		-- MOD.EAP: Hardcode remove rubber plantation from the tech tree until I have a better solution
		if thisImprovement.Type == "IMPROVEMENT_RUBBER_PLANTATION" then
			validImprovementBuilds[thisImprovement.Type] = nil;
		end
...
function AddSmallButtonsToTechButton( thisTechButtonInstance, tech, maxSmallButtons, textureSize, startingButtonNum )
	...
	for row in GameInfo.Improvement_TechYieldChanges( thisTechType ) do
		-- MOD.EAP: Remove improvements that the civ is not allowed to build
		if validImprovementBuilds[row.ImprovementType] == row.ImprovementType then
		-- MOD.EAP: End
	...
	for row in GameInfo.Improvement_TechNoFreshWaterYieldChanges( thisTechType ) do
		...
		-- MOD.EAP: Check if the improvement is valid for this civ
		if validImprovementBuilds[row.ImprovementType] ~= row.ImprovementType then -- skip
		-- MOD.EAP: End
		elseif yield and not addSmallActionButton( GameInfo.Builds{ ImprovementType = row.ImprovementType }(), yield.IconString,
	...
	if bnw_mode then
		...
		if civ5_mode then
			...
			-- MOD.EAP: Globalization custom button
			addSmallGenericButtonIf( (tonumber(tech.AdditionalDelegates) or 0) > 0 and "TXT_KEY_TECH_GLOBALIZATION_EXTRA_VOTES", tech.AdditionalDelegates )
			-- MOD.EAP: End
