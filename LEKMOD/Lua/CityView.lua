...
function AddBuildingButton( pCity, building )
	...
	if (pCity:IsHasBuilding(buildingID)) then
		...
		-- Religious Buildings have special names ~~ Sike Loup.
...
function OnCityViewUpdate()
	...
	if (pCity ~= nil) then
		...
		if workerHeadingOpen then
			...
			elseif focusType == CityAIFocusTypes.CITY_AI_FOCUS_TYPE_GOLDEN_AGE_POINTS then -- v34
				Controls.GoldenAgePointsFocusButton:SetCheck( true );
			else
				Controls.BalancedFocusButton:SetCheck( true );
			end
			...
			if (iProgress > 0) then		
				...
				if(unitClass ~= nil) then
					...
					if iGPPChange > 0 then
						...
						local iBuildingMod = pCity:GetBuildingGreatPeopleRateModifier(pSpecialistInfo.ID);
						...
						local iMod = iPlayerMod + iPolicyMod + iWorldCongressMod + iCityMod + iBuildingMod + iGoldenAgeMod;
						...
						if (iCityMod + iBuildingMod > 0) then
							strToolTipText = strToolTipText .. "[NEWLINE]" .. Locale.ConvertTextKey("TXT_KEY_CITY_GP_MOD", iCityMod + iBuildingMod);
						end
		...
		if (pCity:GetResourceDemanded(true) ~= -1) then
			...
			local iResourceDemandedTimer = pCity:GetResourceDemandedCountdown();
			szResourceDemanded = Locale.ConvertTextKey(pResourceInfo.IconString) .. " " .. Locale.ConvertTextKey(pResourceInfo.Description) .. " (" .. iResourceDemandedTimer .. ")";
...
--		UpdateSpecialists(pCity);
		...
		local iGoldenAgePointsPerTurn = pCity:GetYieldRateTimes100(YieldTypes.YIELD_GOLDEN_AGE_POINTS) / 100;
		Controls.GoldenAgePointsPerTurnLabel:SetText( Locale.ConvertTextKey("TXT_KEY_CITYVIEW_PERTURN_TEXT", iGoldenAgePointsPerTurn) );
		...
		if (UI.IsCityScreenViewingMode()) then
			...
			Controls.GoldenAgePointsFocusButton:SetDisabled( true ); -- v34
			...
			Controls.GoldenAgePointsFocusButton:SetDisabled( false ); -- v34
...
function DoUpdateUpperLeftTooltips()
	...
	local strGoldenAgePointsToolTip = GetGoldenAgePointsTooltip(pCity);
	Controls.GoldenAgePointsBox:SetToolTipString(strGoldenAgePointsToolTip);
...
-- v34 Golden Age Points
Controls.GoldenAgePointsFocusButton:SetVoid1( CityAIFocusTypes.CITY_AI_FOCUS_TYPE_GOLDEN_AGE_POINTS );
Controls.GoldenAgePointsFocusButton:RegisterCallback( Mouse.eLClick, FocusChanged );
