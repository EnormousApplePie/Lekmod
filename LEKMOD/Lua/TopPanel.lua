...
function UpdateData()
	...
	if( iPlayerID >= 0 ) then
		...
		if (pPlayer:GetNumCities() > 0) then
			...
			-----------------------------
			-- Update Golden Age Info altered for v34
			-----------------------------
			local strGoldenAgeStr;
			local iGoldenAgeProgress = pPlayer:GetGoldenAgeProgressMeter();
			local iGoldenAgeThreshold = pPlayer:GetGoldenAgeProgressThreshold();
			local iTotalGoldenAgePointsPerTurn = pPlayer:GetTotalGoldenAgePointsInEmpire(); -- This grabs both Excess Happiness and GAP-as-a-yield
			...
			if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_HAPPINESS)) then
				...
				if (pPlayer:GetGoldenAgeTurns() > 0) then
					...
					-- Transformed into a TXT_KEY with 3 parameters 1_GoldenAgeProgress 2_GoldenAgeThreshold and 3_GoldenAgePointsPerTurn GoldenAgePointsPerTurn changes color if it is negative
					strGoldenAgeStr = Locale.ConvertTextKey("TXT_KEY_TOP_PANEL_GOLDEN_AGE", iGoldenAgeProgress, iGoldenAgeThreshold, iTotalGoldenAgePointsPerTurn);
				end	
...
function HappinessTipHandler( control )
	...
	if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_HAPPINESS)) then
		...
		local iLegaciesHappiness = pPlayer:GetHappinessFromLegacies();
		...
		local iHandicapHappiness = pPlayer:GetHappiness() - iPoliciesHappiness - iLegaciesHappiness - iResourcesHappiness - iCityHappiness - iBuildingHappiness - iTradeRouteHappiness - iReligionHappiness - iNaturalWonderHappiness - iMinorCivHappiness - iExtraHappinessPerCity - iLeagueHappiness;
		...
		local iTotalHappiness = iPoliciesHappiness + iLegaciesHappiness + iResourcesHappiness + iCityHappiness + iBuildingHappiness + iMinorCivHappiness + iHandicapHappiness + iTradeRouteHappiness + iReligionHappiness + iNaturalWonderHappiness + iExtraHappinessPerCity + iLeagueHappiness;
		...
		if (iLegaciesHappiness >= 0) then
			strText = strText .. "[NEWLINE]";
			strText = strText .. "  [ICON_BULLET]" .. Locale.ConvertTextKey("TXT_KEY_TP_HAPPINESS_LEGACIES", iLegaciesHappiness);
		end
		strText = strText .. "[NEWLINE]";
...
-- Golden Age Tooltip altered for v34
function GoldenAgeTipHandler( control )
	...
	if (Game.IsOption(GameOptionTypes.GAMEOPTION_NO_HAPPINESS)) then
		...
		local iHappiness = pPlayer:GetExcessHappiness();
		local iGoldenAgePointsFromCities = pPlayer:GetGoldenAgePointsFromCities(); -- v34
		...
		if (iHappiness >= 0) then
			strText = strText .. Locale.ConvertTextKey("TXT_KEY_TP_GOLDEN_AGE_ADDITION_HAPPINESS", iHappiness);
		else
			strText = strText .. "[COLOR_WARNING_TEXT]" .. Locale.ConvertTextKey("TXT_KEY_TP_GOLDEN_AGE_LOSS_UNHAPPINESS", -iHappiness) .. "[ENDCOLOR]";
		end

		if (iGoldenAgePointsFromCities >= 0) then -- v34
			strText = strText .. "[NEWLINE]";
			strText = strText .. Locale.ConvertTextKey("TXT_KEY_TP_GOLDEN_AGE_ADDITION_CITIES", iGoldenAgePointsFromCities);
		else 
			strText = strText .. "[NEWLINE]";
			strText = strText .. "[COLOR_WARNING_TEXT]" .. Locale.ConvertTextKey("TXT_KEY_TP_GOLDEN_AGE_LOSS_CITIES", -iGoldenAgePointsFromCities) .. "[ENDCOLOR]";
		end
