...
function UpdateCombatOddsUnitVsCity(pMyUnit, pCity)
	...
	if (pMyUnit ~= nil) then
		...
		if (iMyStrength > 0) then
			...
			-- Akkad General City Attack Bonus
			if pMyUnit:IsNearUnitWithPromotion(GameInfoTypes["PROMOTION_GREAT_GENERAL"], 2, false, true) then
				iModifier = pMyPlayer:GetGreatGeneralSiegeBonusFromTraits() + pMyPlayer:GetGreatGeneralSiegeBonusFromLegacies();
				if (iModifier ~= 0) then
					controlTable = g_MyCombatDataIM:GetInstance();
					controlTable.Text:LocalizeAndSetText( "TXT_KEY_TRAIT_GG_SIEGE_BONUS" );
					controlTable.Value:SetText( GetFormattedText(strText, iModifier, true, true) );
				end
			end
...
function UpdateCombatOddsUnitVsUnit(pMyUnit, pTheirUnit)
	...
	if (pMyUnit ~= nil) then
		...
		if (iMyStrength > 0) then
			...
			-- MOD.EAP LEKMOD: GetCombatBonusVsDifferentIdeologyModifier
			-- CombatBonusVsDifferentIdeology
			iModifier = pMyUnit:GetCombatBonusVsDifferentIdeologyModifier();
			print("Different Ideology Combat Bonus My Unit Gets: " .. pMyUnit:GetCombatBonusVsDifferentIdeologyModifier());
			if (iModifier ~= 0) then
				if (pMyPlayer:GetLateGamePolicyTree() ~= -1 
				and pTheirPlayer:GetLateGamePolicyTree() ~= -1 
				and pMyPlayer:GetLateGamePolicyTree() ~= pTheirPlayer:GetLateGamePolicyTree()) then

					controlTable = g_MyCombatDataIM:GetInstance();
					controlTable.Text:LocalizeAndSetText( "TXT_KEY_EUPANEL_DIFFERENT_IDEOLOGY_BONUS" );
					controlTable.Value:SetText(GetFormattedText(strText, iModifier, true, true));
	...
	----				strString.append(GetLocalizedText("TXT_KEY_COMBAT_PLOT_MOD_VS_TYPE", iModifier, GC.getUnitClassInfo(pMyUnit:getUnitClassType()).GetTextKey()));
				...
				---- MOD.EAP LEKMOD: GetCombatBonusVsDifferentIdeologyModifier
				-- CombatBonusVsDifferentIdeology
				iModifier = pTheirUnit:GetCombatBonusVsDifferentIdeologyModifier();
				print("Different Ideology Combat Bonus Their Unit Gets: " .. pTheirUnit:GetCombatBonusVsDifferentIdeologyModifier());
				if (iModifier ~= 0) then
					if (pTheirPlayer:GetLateGamePolicyTree() ~= -1 
					and pMyPlayer:GetLateGamePolicyTree() ~= -1 
					and pTheirPlayer:GetLateGamePolicyTree() ~= pMyPlayer:GetLateGamePolicyTree()) then
...
--				strString.append(GetLocalizedText("TXT_KEY_COMBAT_PLOT_EXTRA_STRENGTH", iModifier));
		...
		---- MOD.EAP LEKMOD: GetCombatBonusVsDifferentIdeologyModifier
				-- CombatBonusVsDifferentIdeology
		iModifier = theirUnit:GetCombatBonusVsDifferentIdeologyModifier();
		print("Different Ideology Combat Bonus Their Unit Gets: " .. theirUnit:GetCombatBonusVsDifferentIdeologyModifier());
		if (iModifier ~= 0) then
			if (theirPlayer:GetLateGamePolicyTree() ~= -1 
			and myPlayer:GetLateGamePolicyTree() ~= -1 
			and theirPlayer:GetLateGamePolicyTree() ~= myPlayer:GetLateGamePolicyTree()) then
